...
class PointCloudServiceImpl final : public ProcessPointCloud::Service {
  ...
  Status VoxelGrid(ServerContext *context, const VoxelFilterInput *input,
    ...
    std::cout << "Applying Voxel Grid to " << input_pcl->size() << " points with resolution "
              << input->resolution() << ".\n";
  ...
  Status VoxelMax(ServerContext *context, const VoxelFilterInput *input,
                  PointCloud *output) override {
    duna::VoxelMax<PointT> voxel;
    pcl::PointCloud<PointT>::Ptr input_pcl = pcl::make_shared<pcl::PointCloud<PointT>>();
    pcl::PointCloud<PointT>::Ptr output_pcl = pcl::make_shared<pcl::PointCloud<PointT>>();
    duna::GRPCToPCL(input->pointcloud(), *input_pcl);
    std::cout << "Applying Voxel Max to " << input_pcl->size() << " points with resolution "
              << input->resolution() << ".\n";
    voxel.setInputCloud(input_pcl);
