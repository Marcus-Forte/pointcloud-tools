...
class VoxelMax : public pcl::Filter<PointT> {
 ...
 public:
  ...
  virtual ~VoxelMax() { leaves_.clear(); }

  inline void setMaximumPointsPerVoxel(int max_points_per_voxel) {
    max_points_per_voxel_ = max_points_per_voxel;
  }

  inline int getMaximumPointsPerVoxel() const { return max_points_per_voxel_; }
 ...
 protected:
  /** \brief Simple structure to hold an nD centroid and the number of points in
   * a leaf. */
  struct Leaf {
    Leaf() {}
    pcl::Indices indices;
