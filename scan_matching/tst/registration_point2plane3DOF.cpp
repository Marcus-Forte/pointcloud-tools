...
class RegistrationPoint2Plane3DOF : public ::testing::Test {
 ...
 protected:
  ...
  duna_optimizer::LevenbergMarquadt<Scalar, 3> optimizer;
};
...
TYPED_TEST(RegistrationPoint2Plane3DOF, DificultRotation) {
  ...
  // Act
  typename duna_optimizer::ScanMatching3DOFPoint2Plane<PointT, PointT, TypeParam>::Ptr
      scan_matcher_model;
  scan_matcher_model.reset(
      new duna_optimizer::ScanMatching3DOFPoint2Plane<PointT, PointT, TypeParam>(
          this->source, this->target, this->target_kdtree));
  auto cost = new duna_optimizer::CostFunctionNumerical<TypeParam, 3, 1>(scan_matcher_model,
                                                                         this->source->size());
